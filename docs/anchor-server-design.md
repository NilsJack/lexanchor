...
The first session-control layer stores hashed session tokens in SQLite and resolves request scope from bearer tokens before using development headers. This improves operational safety for local/team testing but should be replaced by real identity-provider authentication, membership checks, and RBAC before production deployment.

The first RBAC layer uses a built-in role permission matrix from session roles. It covers scan, redaction, job/artifact/action access, audit reads, ruleset reads, and rule-draft creation. This is intentionally concise and should later be replaced by persisted organization/workspace/matter memberships and role assignments.

The next hardening step adds SQLite membership records keyed by user, organization, and workspace. Session roles are derived from membership records at login time. Development login can bootstrap a membership when none exists, but production should replace this with identity-provider users and administrator-managed membership assignment.

...
### Minimal RBAC hardening

- Added a built-in role permission matrix for `org_admin`, `legal_reviewer`, `rule_author`, `auditor`, `business_submitter`, and `development_header_scope`.
- Added permission checks to scan, redaction, job/artifact/action, audit, ruleset, and rule-draft endpoints.
- Development header fallback keeps all permissions through `development_header_scope` for local compatibility.
- This is a concise RBAC layer, not persisted membership-backed authorization.

### Membership-backed RBAC

- Added SQLite-backed membership records keyed by user, organization, and workspace.
- Added `POST /api/v1/memberships/dev-grant` and `GET /api/v1/memberships`.
- `dev-login` now derives session roles from membership records when present.
- If no membership exists, `dev-login` creates a development bootstrap membership from requested roles for local setup.
- This moves RBAC from purely request-provided roles to workspace membership records, while matter-level assignment remains future work.

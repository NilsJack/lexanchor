...
## Project Setup and Build Habits

This workspace currently has no git repository available. Before large edits, create a timestamped backup under `back/` from the LexAnchor project root.

Current backup command pattern:

```bash
cd /Users/lq/projects/laws/lexAnchor
backup_dir="back/lexAnchor_backup_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$backup_dir"
rsync -a \
	--exclude 'back/' \
...
Latest known backup created during this session:

```text
back/lexAnchor_backup_20260501_070220
```

Development habits that matter in this repo:

- Use `uv` for dependency and command execution.
- Use `--python 3.11` explicitly when running tests or server commands.
- Keep proxy variables on dependency and test commands: `HTTP_PROXY=http://127.0.0.1:51809`, `HTTPS_PROXY=http://127.0.0.1:51809`, `UV_HTTP_TIMEOUT=120`.
- Set `PYTHONDONTWRITEBYTECODE=1` when running tests to avoid persistent `__pycache__` churn.
- Clean generated runtime state after smoke tests when it is not needed: `rm -rf storage && find app tests -type d -name __pycache__ -prune -exec rm -rf {} +` from `server/`.
- Prefer the API/service tests and HTTP smoke loop over manual file inspection for behavior validation.

Run the development server from `server/`:
...
For local HTTP smoke tests, bypass the proxy for localhost:

```bash
NO_PROXY=127.0.0.1,localhost
```

Minimal v1.0 runtime smoke path:

```text
POST  /api/v1/contract/scan-text using rules_v1.0
...
GET   /api/v1/audit/events?resource_type=action&resource_id={action_id}
```

The latest HTTP smoke loop verified: scan produced actions, `AA-0001` was updated to `completed`, and audit events were visible for both the job and action scopes.
...
- Lightweight server-side session tokens plus request-scope header fallback.
...
- Rule-authoring draft generation from lawyer guidance text.
- Lawyer operation UI recommendation endpoint.

## Access Isolation

Current isolation supports lightweight server-side sessions. Preferred request auth is:

```text
Authorization: Bearer {session_token}
```

Alternative session header:

```text
X-LexAnchor-Session: {session_token}
```

Development header fallback remains available and receives `development_header_scope`, but is not production authentication:
...
Session tokens are stored hashed in SQLite and carry `user_id`, `organization_id`, `workspace_id`, roles, expiry, and revocation state. Session roles are derived from SQLite membership records at login time. Each job is bound to `created_by`, `organization_id`, and `workspace_id`. Job metadata, artifacts, actions, and audit reads return `404` or empty scoped results when accessed from a different scope. This avoids leaking object existence through job ids.

## Minimal RBAC

Current RBAC is a small built-in role permission matrix evaluated from `RequestScope.roles`. Roles are loaded from membership records when a session is created.
...
development_header_scope: all permissions for local compatibility
```

Membership behavior:

- Membership table key: `user_id`, `organization_id`, `workspace_id`.
- `POST /api/v1/memberships/dev-grant` creates or updates membership roles.
- `GET /api/v1/memberships` lists memberships for a manager scope.
- `POST /api/v1/sessions/dev-login` uses membership roles if a record exists.
- If no membership exists, `dev-login` creates a development bootstrap membership from requested roles.

Implemented permission checks cover:

- Contract scan endpoints: `contract:scan`.
- Redaction endpoint: `document:redact`.
- Job read endpoint: `job:read`.
- Report/PDF artifact endpoints: `artifact:read`.
- Action list/update endpoints: `action:read`, `action:update`.
- Audit event endpoints: `audit:read`.
- Ruleset endpoints: `rule:read`.
- Rule draft endpoint: `rule_draft:create`.
- Membership endpoints: `membership:read`, `membership:write`.

This is now membership-backed at organization/workspace scope, but not yet matter-assignment-backed. Future work should add matter/project-level memberships and derive roles from real identity-provider users instead of development login.

The current runtime has a lightweight SQLite audit log for session creation/revocation, scan lifecycle, job/artifact reads, action reads, and action updates. Full workgroup permissions, role-based access, membership tables, matter-level assignments, and immutable production audit retention are future work.
...
## Rule Authoring Automation

Current implementation:

- Endpoint: `POST /api/v1/rule-authoring/draft-from-text`.
- Input: `guide_text`, `rule_scope`, `scope_id`, optional `source_name`, and `max_rules`.
- Output: disabled LexAnchor-compatible rule drafts, review checklist, warnings, and `draft_id`.
- Audit event: `rule_draft.created` scoped by request headers.
- Generated rules set `enabled: false` and `draft_status: needs_lawyer_review` by default.

The generator is deterministic and local. It extracts candidate review segments, legal keywords, severity, category, rule type, negative filters, and recommendations. It does not call external LLMs and does not write generated rules into production YAML files.

Activation should remain a separate lawyer-reviewed workflow. Future implementation should persist drafts in a rule-draft table, add reviewer approval, test examples, versioning, and promotion into personal/team/company rule libraries.

## Lawyer Operation UI Recommendations

Endpoint: `GET /api/v1/rule-authoring/lawyer-ui-recommendations`.

The response is structured for a future frontend or agent and includes:
...
- Server hardening priorities.

The UI should be a dense legal operations workbench, not a landing page. Primary visible concepts should be document, anchors, action queue, rule drafts, and audit timeline.
...
- Query endpoints: `GET /api/v1/audit/events` with optional `resource_type`, `resource_id`, and `limit`; `GET /api/v1/jobs/{job_id}/audit-events` for job-scoped audit retrieval after job access checks.
...
- `server/app/services/rule_authoring.py`: deterministic rule draft generation from lawyer guidance text.
...
- RuleDraftGenerator disabled draft rule generation.
...
At the time rule-authoring automation was added, the command above passed with 24 tests.
...
- Access isolation uses server sessions, membership-backed roles, and development header fallback; production identity-provider auth remains future work.
- Session control and RBAC are lightweight and local; real identity-provider login and matter assignments remain future work.
...
- Rule draft generation is deterministic and local; it creates reviewable drafts but does not yet persist approval state or activate rules.

## Server Hardening Priorities

Before real team deployment, harden these areas:

- Replace development login and local membership management with real identity-provider login and matter-level role checks.
- Persist rule drafts separately with reviewer approval, versioning, test examples, and activation workflow.
- Move Action Anchors from job result JSON into a dedicated actions table with state transitions, assignment, due dates, and comments.
- Make audit logs append-only with retention/export controls and auditor-role read access.
- Move from SQLite/local filesystem to PostgreSQL and object storage for multi-user deployments.
- Add input size limits and rate limits for scan and rule-authoring endpoints.
- Add generated-rule collision checks, YAML schema validation, and pre-activation scan tests.
- Add background workers or a durable queue for long document extraction, OCR, rendering, and future LLM validation.

## Roadmap Direction

Near-term next work:

- Keep the 27-test suite passing as session control, membership-backed RBAC, v1.0 action workflow, and rule-authoring fields expand.

...
from app.services.access_control import RequestScope, assert_job_access, scope_from_request
...
def get_job(job_id: str, request: Request) -> dict:
    job, scope = _scoped_job_with_scope(job_id, request)
...
def list_job_audit_events(job_id: str, request: Request, limit: int = 100) -> dict:
    _job, scope = _scoped_job_with_scope(job_id, request)
...
def _scoped_job(job_id: str, request: Request) -> dict:
    job, _scope = _scoped_job_with_scope(job_id, request)
    return job


def _scoped_job_with_scope(job_id: str, request: Request) -> tuple[dict, RequestScope]:

...
Session tokens are stored hashed in SQLite and carry `user_id`, `organization_id`, `workspace_id`, roles, expiry, and revocation state. Each job is bound to `created_by`, `organization_id`, and `workspace_id`. Job metadata, artifacts, actions, and audit reads return `404` or empty scoped results when accessed from a different scope. This avoids leaking object existence through job ids.

## Minimal RBAC

Current RBAC is a small built-in role permission matrix evaluated from `RequestScope.roles`.
...
This is not yet membership-backed RBAC. Future work should persist organization/workspace/matter memberships and derive roles from assignments instead of trusting roles provided during development login.
...
- Session control and RBAC are lightweight and local; real identity-provider login, membership checks, persisted roles, and matter assignments remain future work.
...
- Replace development login and built-in RBAC with real identity-provider login, persisted memberships, and matter-level role checks.
...
- Keep the 26-test suite passing as session control, RBAC, v1.0 action workflow, and rule-authoring fields expand.

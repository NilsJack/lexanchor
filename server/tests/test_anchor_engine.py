...
from app.services.access_control import RequestScope, assert_job_access, assert_permission, has_permission
...
from app.services.membership_store import MembershipStore
...
class AnchorEngineTests(unittest.TestCase):
    ...
    def test_session_store_hashes_token_and_revokes_session(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            store = SessionStore(Path(tmp_dir) / "sessions.sqlite3")
            ...
            self.assertTrue(created["session_token"].startswith("lxs_sess_"))
            session = store.get_session(created["session_token"])
            self.assertIsNotNone(session)
            self.assertEqual(session["user_id"], "lawyer_a")
            self.assertIn("rule_author", session["roles"])
            revoked = store.revoke_session(created["session_token"])
            self.assertEqual(revoked["session_id"], created["session_id"])
            self.assertIsNone(store.get_session(created["session_token"]))

    def test_membership_store_grants_and_updates_roles(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            store = MembershipStore(Path(tmp_dir) / "memberships.sqlite3")
            created = store.grant_membership(
                user_id="lawyer_a",
                organization_id="org_a",
                workspace_id="workspace_a",
                roles=["legal_reviewer"],
            )
            self.assertTrue(created["membership_id"].startswith("mbr_"))
            self.assertEqual(created["roles"], ["legal_reviewer"])

            updated = store.grant_membership(
                user_id="lawyer_a",
                organization_id="org_a",
                workspace_id="workspace_a",
                roles=["rule_author", "legal_reviewer"],
            )
            self.assertEqual(updated["membership_id"], created["membership_id"])
            membership = store.get_membership(user_id="lawyer_a", organization_id="org_a", workspace_id="workspace_a")
            self.assertEqual(membership["roles"], ["rule_author", "legal_reviewer"])
            self.assertEqual(len(store.list_memberships(organization_id="org_a", workspace_id="workspace_a")), 1)

    def test_minimal_rbac_permissions_allow_and_deny_by_role(self) -> None:
        reviewer = RequestScope(user_id="lawyer_a", organization_id="org_a", workspace_id="workspace_a", roles=("legal_reviewer",))
        rule_author = RequestScope(user_id="author_a", organization_id="org_a", workspace_id="workspace_a", roles=("rule_author",))
        submitter = RequestScope(user_id="submitter_a", organization_id="org_a", workspace_id="workspace_a", roles=("business_submitter",))
        dev_scope = RequestScope(user_id="dev", organization_id="org_a", workspace_id="workspace_a", roles=("development_header_scope",))

        self.assertTrue(has_permission(reviewer, "action:update"))
        self.assertTrue(has_permission(rule_author, "rule_draft:create"))
        self.assertTrue(has_permission(dev_scope, "action:update"))
        self.assertFalse(has_permission(submitter, "action:update"))
        self.assertFalse(has_permission(rule_author, "contract:scan"))

        assert_permission(reviewer, "contract:scan")
        with self.assertRaises(HTTPException) as submitter_error:
            assert_permission(submitter, "action:update")
        self.assertEqual(submitter_error.exception.status_code, 403)

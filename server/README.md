...
The server also supports lightweight session tokens. Create a development session and then send the token as `Authorization: Bearer ...`:
...
Session tokens are stored as hashes in SQLite and can be revoked with `DELETE /api/v1/sessions/current`. Header scopes remain as a local development fallback, not a production auth model.

## Minimal RBAC

The current server enforces a small built-in role permission matrix from session roles:
...
This is intentionally simple. It blocks obvious role misuse while keeping local development workable. Full production RBAC still needs persisted organization/workspace/matter memberships and real identity-provider login.

Roles are now derived from SQLite membership records when sessions are created. `dev-login` bootstraps a membership only when none exists for that user/org/workspace. A workspace manager or development header scope can grant/update membership records:
...
Rule authoring draft generation:
...
Generated rules are disabled by default and must be reviewed before activation. UI planning data is available at:
